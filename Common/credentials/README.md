...
GPG is invoked directly by RWR. No shell script or jq orchestrates credential work. The provider, attachment bindings, passphrase declaration, fingerprint, and native tasks all live in `gpg.cue`; edit that one shared file when rotating keys. Ultimate ownertrust and Git commit signing are explicitly requested by the restore task, preserving the previous personal workflow.
