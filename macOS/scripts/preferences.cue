...
// scripts rather than file targets.
{
	"scripts": [
		...
		// A plist import can set Rectangle's launchOnLogin preference, but macOS
		// does not register the app as a login item until Rectangle itself runs.
		// The import script only restores an already-running instance, so a fresh
		// install otherwise remains stopped and unregistered.
		{
			"action":  "run"
			"content": "#!/bin/bash\nset -euo pipefail\n\nif ! pgrep -qx Rectangle; then\n\topen -a Rectangle\nfi\n"
			"exec":    "bash"
			"name":    "ensure_rectangle_running"
		},
		{
			"action":  "run"
